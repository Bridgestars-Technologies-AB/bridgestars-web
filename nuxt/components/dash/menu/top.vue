...
<template>
  <div class="flex justify-end bg-dash-light-400 dark:bg-dash-dark-200 pr-3 transition-colors duration-300">
...
<!-- side menu open/close button with tooltip, some styles going on here for all the different situations -->
    <div class="absolute left-0 z-10">
      <div :class="`flex justify-center h-full w-[67px] z-10 ${open ? 'ml-[200px]' : 'sm:dark:bg-dash-dark-100 sm:bg-dash-light-300'} transition-[margin-left,background-color,color] duration-300`" @click="open = !open">

        <base-tooltip :text="`${open ? 'Stäng sidomenyn' : 'Öppna sidomenyn'}`" position="right" class="flex items-center">
          <span v-if="open" class="i-mdi-menu-open dark:bg-dash-light-300 bg-dark h-[38px] w-[38px] ml-3 transition-colors duration-300"></span>
          <span v-else class="i-mdi-menu-close dark:bg-dash-light-300 bg-dark h-[38px] w-[38px] ml-3 transition-colors duration-300"></span>
        </base-tooltip>
      </div>
    </div>
    ...
    <div class="flex items-center space-x-4 py-1" >
      <base-tooltip class="flex items-center" :text="$t('dashboard:top_menu.tooltip.toggle_dark_mode')">
          <span
            :class="`${
              !darkMode.enabled
        ...
        <base-tooltip class="flex items-center" :text="$t('dashboard:top_menu.tooltip.settings')">
          <span
