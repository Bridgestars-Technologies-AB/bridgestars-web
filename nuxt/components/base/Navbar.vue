...
<template>
  <header class="w-full">
    <nav class="flex xs:flex-wrap md:flex-nowrap justify-between items-center w-[100%] bg-white py-4 px-2 mb-2 rounded-2xl">

<!-- Logo -->
    <div class="flex items-center md:w-auto">
      <img src="~/assets/bridgestars/logo/logo-trans-128px.png" 
          class="h-[32px] w-[32px] mx-2">
      <h3 class="text-[22px]">Bridgestars</h3>
    </div>

<!-- Open Menu Button -->
      <div class="flex items-center md:hidden mr-2">
        <HamburgerMenuButton  @click="isOpen = !isOpen" :isOpen="isOpen" class="!scale-[0.4]"/>
            ...
            <nuxt-link :to="route.path" class="text-[14px]">
              ...
              <div class="flex items-center space-x-2 mx-1 xs:mt-3 md:mt-0">
                <component :is="route.icon" class="scale-[1.3]"
                  :style="'color: ' + (route.success ? success : 'rgb(120,120,120)')"/>
                
                <span 
                  :class="'text2 !text-[18px] !font-medium ' + 
                    (route.success ? '!text-[#59BA83]' : '')"> 
                  {{ route.name }}
                </span>
              </div>
