...
const data = ref(null)
const order = ["signIn", "searchUsers", "getUserInfo", "sendFriendRequest","acceptFriendRequest", "denyFriendRequest", "removeFriend", "redeemVoucher", "generateSubscriptionDashboardLink", "archivePost", "fetchIfUpdated", "signOutFromAllDevices", "assignRole", "assignRoomMod"]
onMounted(() => {
  //reload data() {
  ...
  <!--  all classes links -->
      ...
      <div v-if="data" class="mx-9 my-9 flex flex-wrap flex-col">
        <div v-for="table in data" class="relative flex items-center text-center">
          ...
          <span class="xs:text-[14px] md:text-[20px] font-family2 opacity-0">{{table['name']}}</span>
            <div v-for="role in table['roles']" class="ml-[90px] rounded-xl px-[5px] py-[0px] bg-red-400">
              <span class="text1 text-white font-medium text-[12px]">{{role}}</span>
            </div>
          </div>
        </div>
      </div>

    <div v-if="data" class="flex flex-col items-center w-full flex-grow space-y-[80px] pb-[350px]">
      <div v-for="item in data" class="flex flex-col px-3 items-start mt-5 w-full max-w-[1700px]">
        <div class="flex items-center cursor-pointer mb-0" @click="copyUrl(item['_id'])">
          ...
          <div v-for="role in item['roles']" class="ml-3 rounded-xl px-2 py-1 bg-red-400">
            <span class="text1 text-white font-medium text-[20px]">{{role}}</span>
          </div>
        </div>
        <template v-if="item.desc">
          <div v-for="d in (item.desc).split(';')" class="sm:h-[20px] xs:text-[11px] mt-1">
