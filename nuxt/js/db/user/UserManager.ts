...
export default class UserManager {
  ...
  getStatus = (id: string) => {
    ...
    if (
      !user ||
      (Date.now() - user.statusFetchTime.getTime() > 1000 * 60 &&
        !isSubscribedToStatusUpdate)
    ) {
      //if status is 60 seconds old then request the new one, this stops looped rerender
      if (!user)
